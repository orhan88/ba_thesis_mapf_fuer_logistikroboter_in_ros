...
class ConfigEntry(object):
    """
    This class represents an entry in the configuration of the simulation.  
    Every entry stores the following values: 
    -   display name
    -   default value
    -   current value
    -   and to_string() method 
    """

    def __init__(self, default_value, to_string, display_name = None):
...
class Configuration(object):
    """
    This class provides a configuration of the simulation with default values. 
    """
    ...
    def init_defaults(self, args = None):
        """
        This function sets the variables in the configuration to default values.
        There are multiple sections of options like "color" or "display".
        Every section contains several options which can be changed.
        
        :param args: -  command line arguments (Not used!)
        """       
        self._values = {
                        ('color', 'color_start_node') : ConfigEntry(0x000000, hex, 'color start node'),
                        ('color', 'color_start_node_highlighted') : ConfigEntry(0x000000, hex, 'color start node highlighted'),
                        ('color', 'color_travelled_path_highlighted') : ConfigEntry(0x3adfc, hex, 'color travelled path highlighted'),
                        ('color', 'color_future_path_highlighted') : ConfigEntry(0x40a832, hex, 'color future path highlighted'),
                        #('color', 'color_robot_ros0') : ConfigEntry(0xff0000, hex, 'color robot_ros 1'),
                        ('color', 'color_robot_ros0') : ConfigEntry(0xffff00, hex, 'color robot_ros 1'),
                        ('color', 'color_robot_ros1') : ConfigEntry(0xff0000, hex, 'color robot_ros 2'),
                        ('color', 'color_checkpoint_ros0') : ConfigEntry(0xffffff, hex, 'color checkpoint_ros 1'),
                        ('color', 'color_checkpoint_ros1') : ConfigEntry(0xffffff, hex, 'color checkpoint_ros 2'),
                        #('color', 'color_checkpoint_ros0') : ConfigEntry(0x009b00, hex, 'color checkpoint_ros 1'),
                        #('color', 'color_checkpoint_ros1') : ConfigEntry(0x009bff, hex, 'color checkpoint_ros 2'),
                        #('color', 'color_disabled_node') : ConfigEntry(0x323232, hex, 'color disabled nodes'),
                        ('color', 'color_disabled_node') : ConfigEntry(0xabdbe3, hex, 'color disabled nodes'),
                        
                        ('display', 'id_font_scale') : ConfigEntry(1.0, str, 'id font scale'),
                        ('display', 'id_font_bold') : ConfigEntry(False, str, 'id font bold'),
                        ('display', 'id_font_color') : ConfigEntry(0x000000, hex, 'id font color')
                        }
        """
        #Color
        ('color', 'color_robot0') : ConfigEntry(0xffff00, hex, 'color robot 1'),
        ('color', 'color_robot1') : ConfigEntry(0xff0000, hex, 'color robot 2'),
        ('color', 'color_shelf0') : ConfigEntry(0x009b00, hex, 'color shelf 1'),
        ('color', 'color_shelf1') : ConfigEntry(0x009bff, hex, 'color shelf 2'),
        ('color', 'color_shelf2') : ConfigEntry(0xff0000, hex, 'color carried shelf'),
        ('color', 'color_pickingstation0') : ConfigEntry(0xffff00, hex, 'color picking station 1'),
        ('color', 'color_pickingstation1') : ConfigEntry(0xffffff, hex, 'color picking station 2'),
        ('color', 'color_chargingstation0') : ConfigEntry(0x202020, hex, 'color charging station 1'),
        ('color', 'color_chargingstation1') : ConfigEntry(0xff2020, hex, 'color charging station 2'),
        ('color', 'color_checkpoint0') : ConfigEntry(0x006400, hex, 'color checkpoint 1'),
        ('color', 'color_checkpoint1') : ConfigEntry(0x006464, hex, 'color checkpoint 2'),
        ('color', 'color_checkpoint2') : ConfigEntry(0x646464, hex, 'color checkpoint 3'),
        ('color', 'color_checkpoint3') : ConfigEntry(0x646400, hex, 'color checkpoint 4'),
        ('color', 'color_highway') : ConfigEntry(0x9b9bc8, hex, 'color highways'),

        #Zoom          
        ('controls', 'zoom_out') : ConfigEntry('-', str, 'zoom out'),
        ('controls', 'zoom_in') : ConfigEntry('+', str, 'zoom in'),

        #Step
        ('visualizer', 'step_time') : ConfigEntry(1200, str, 'step time'),
        
        #Controls
        ('controls', 'step_speed_up') : ConfigEntry('Right', str, 'speed up'),
        ('controls', 'step_slow_down') : ConfigEntry('Left', str, 'slow down'),
        ('controls', 'do_step') : ConfigEntry('Up', str, 'do one step'),
        ('controls', 'do_backstep') : ConfigEntry('Down', str, 'undo one step'),
        ('controls', 'pause') : ConfigEntry('Space', str, 'pause'),
        """
    
    def get(self, section, option):
        """
        Return the value of a specific option from the configuration.   
        There are multiple sections of options in the configuration ("color", "display" ..).  
        Every section contains several options which can be changed.  

        :param section:     str -   The section to which the option belongs
        :param option:      str -   The option whose value should be returned
        :return: The value of the option in the configuration
        """   
        try:
...
class Configuration(object):
    def __init__(self, args = None):
        ...
        '''
        #create /home/username/.config/asprilo
        if not os.path.isdir(os.path.expanduser('~/.config/asprilo')):
            os.makedirs(os.path.expanduser('~/.config/asprilo'))

        #create /home/username/.config/asprilo/visualizer
        if not os.path.isdir(os.path.expanduser('~/.config/asprilo/visualizer')):
            os.makedirs(os.path.expanduser('~/.config/asprilo/visualizer'))
    ...
    def init_defaults(self, args = None):
        self._values = {
                        ('color', 'color_start_node') : ConfigEntry(self._read_hex_from_config, 0x000000, hex, 'color start node'),
                        ('color', 'color_start_node_highlighted') : ConfigEntry(self._read_hex_from_config, 0x000000, hex, 'color start node highlighted'),
                        ('color', 'color_travelled_path_highlighted') : ConfigEntry(self._read_hex_from_config, 0x3adfc, hex, 'color travelled path highlighted'),
                        ('color', 'color_future_path_highlighted') : ConfigEntry(self._read_hex_from_config, 0x40a832, hex, 'color future path highlighted'),
                        #('color', 'color_robot_ros0') : ConfigEntry(self._read_hex_from_config, 0xff0000, hex, 'color robot_ros 1'),
                        ('color', 'color_robot_ros0') : ConfigEntry(self._read_hex_from_config, 0xffff00, hex, 'color robot_ros 1'),
                        ('color', 'color_robot_ros1') : ConfigEntry(self._read_hex_from_config, 0xff0000, hex, 'color robot_ros 2'),
                        ('color', 'color_checkpoint_ros0') : ConfigEntry(self._read_hex_from_config, 0xffffff, hex, 'color checkpoint_ros 1'),
                        ('color', 'color_checkpoint_ros1') : ConfigEntry(self._read_hex_from_config, 0xffffff, hex, 'color checkpoint_ros 2'),
#                        ('color', 'color_checkpoint_ros0') : ConfigEntry(self._read_hex_from_config, 0x009b00, hex, 'color checkpoint_ros 1'),
#                        ('color', 'color_checkpoint_ros1') : ConfigEntry(self._read_hex_from_config, 0x009bff, hex, 'color checkpoint_ros 2'),
                        ('color', 'color_robot0') : ConfigEntry(self._read_hex_from_config, 0xffff00, hex, 'color robot 1'),
                        ('color', 'color_robot1') : ConfigEntry(self._read_hex_from_config, 0xff0000, hex, 'color robot 2'),
                        ...
                        ('color', 'color_disabled_node') : ConfigEntry(self._read_hex_from_config, 0xabdbe3, hex, 'color disabled nodes'),
