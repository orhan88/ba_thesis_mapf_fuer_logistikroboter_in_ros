...
**restart**, **on_step_update** and **on_step_undo**.
...
class VisualizerItem(object):
    """
    This is the template class for visualizer item.
    ...
    **Attributes**:
    ...
    def __init__(self, ID = 0):
        """
        :param ID: int, optional -  The ID of the object
        """
        self._kind_name = ''
        self._id = ID
        self._model = None
    ...
    def set_model(self, model):
        ...
        :param model: model.Model   -   The model this object belongs to.
        """
    ...
    def get_kind_name(self):
        """
        Returns the kind name of the object.

        :return: str - The kind name of the object
        """
    ...
    def on_step_update(self, time_step):
        ...
        :param time_step: int - the current time step.
        """
        pass
...
class TaskROS(VisualizerItem):
    ...
    **Attributes**:  
    -   **_kind_name**: str                         -   The name of this kind of object. This is always 'task_ros'  
    -   **_robot**:  visualizerGraphicItem.Robot    -   The robot to fulfill the task  
    -   **_changed**: bool                          -   Saves whether this request was edited at the last time step  
    -   **_checkpoints**: a list of visualizerGraphicItem.Checkpoint  -   A list of checkpoints  
    -   **_open**:  -   a list of tuples that contains the name and the checkpoint.
    -   **_history**:   -   a list of visited checkpoints
    """
    ...
    def set_robot(self, robot):
        ...
        :param robot: visualizerGraphicItem.RobotROS -  the robot to which this task should be assigned
        """
    ...
    def get_robot(self):
        """
        Returns the assigned robot - **visualizerGraphicItem.RobotROS**
        """
        return self._robot

    def get_robot_id(self):
        """
        Returns the id of the assigned robot - **int**
        """
    ...
    #CHECKPOINTS
    def add_checkpoint(self, checkpoint_id, name):
    #def add_checkpoint(self, checkpoint_id, name, checkpoint_number):
        """
        Adds a new checkpoint to the task.

        :param checkpoint_id: int -  The id of the checkpoint
        :param name: str -  The name of the checkpoint
        """
        checkpoint = self._model.get_item('checkpoint_ros', checkpoint_id, False, False)
        #exit if no checkpoint exists with the given checkpoint_id
        if checkpoint is None:
            return
        
        #append the checkpoint to the list
        self._checkpoints.append( (checkpoint, name) )

        '''
        #insert the checkpoint to the open list at the right location
        #checkpoint_number = The chronological number of the checkpoint
        index = 0
        ...
        checkpoint_number = len(self._open)
        index = checkpoint_number

        #insert the item at index = index, item = (name, checkpoint_number)
        self._open.insert(index, (name, checkpoint_number))
        #item[0] = name
        #item[1] = checkpoint number
        
    def visit_checkpoint(self, checkpoint):
        """
        This function is called when the assigned robot to this task has reached a checkpoint.

        :param checkpoint: visualizerGraphicItem.Checkpoint -  The checkpoint that was reached
        """
            ...
            if checkpoint2[0] == checkpoint:
                ...
                if len(self._open) > 0:
                    ...
                    if checkpoint2[1] == self._open[0][0]: 
                        ...
                        self._open.pop(0)
                        #[save in history as "the name + (*)"", meaning we visit the right(next needed) checkpoint]
                        self._history.append(checkpoint2[1] + '(*)')
                        checkpoint.visit()
                    else:
                        #[just save the name in history]
                        self._history.append(checkpoint2[1])
                    return
                else:
                    #[just save the name in history]
    ...
    def unvisit_checkpoint(self, checkpoint):
        """
        This function will be called if a time step was undone 
        in which a checkpoint was reached.

        :param checkpoint: visualizerGraphicItem.Checkpoint -  The checkpoint that was reached
        """
        for checkpoint2 in reversed(self._checkpoints):
            #[every checkpoint2 = tuple of (checkpoint, name) ]
            #[IF checkpoints are equal]
            if checkpoint2[0] == checkpoint and len(self._history) > 0:

                #[If the checkpoint name is saved as last element in history] 
                ...
                #[If the checkpoint name is saved as last element in history] + with (*) as Extension 
                #[then delete it from history]
                # AND add it to Open List
                elif self._history[len(self._history)-1] == (checkpoint2[1] + '(*)'):
                    self._history.pop()

                    #insert the item at index = 0, item = (name, checkpoint_number)
                    self._open.insert(0, (checkpoint2[1], 0))
                    checkpoint.unvisit()
                    self._changed = True
                    return        
    ...
    def unvisit_all_checkpoints(self):
        """
        This function deletes the history of visited checkpoints and marks every checkpoint assigned to this task as not visited
        """
        self._open = []
        self._history = []
        ...
        for checkpoint, name in self._checkpoints:
    ...
    def get_changed(self):
        ...
        Returns true if a checkpoint was reached.
        """
    ...
    def get_checkpoints(self):
        ...
        :return: a list of tuples, where every tuple is (checkpoint, name)   
        = (visualizerGraphicItem.CheckpointROS, str)
        """
    ...
    def get_checkpoint_history(self):
        ...
        :return: the list of names of visited checkpoints (list of str)
        """
    ...
    def get_open_checkpoints(self):
        ...
        :return: the list of names of unvisited checkpoints (list of str)
        """
        ll = []
