...
vector<mapf_interfaces::msg::Position> PBS::getPath(int agent) const
{
    vector<mapf_interfaces::msg::Position> path;
    
    for (const auto & t : *paths[agent])
    {
        auto position = mapf_interfaces::msg::Position();
        position.pos_x = search_engines[0]->instance.getColCoordinate(t.location)+1;
        position.pos_y = search_engines[0]->instance.getRowCoordinate(t.location)+1;
        position.theta=0;
        path.push_back(position);
    }
    return path;
    /*
    position.pos_x = search_engines[0]->instance.getColCoordinate((*paths[agent]).at().location)+1;
    position.pos_y = search_engines[0]->instance.getRowCoordinate((*paths[agent]).at().location)+1;
    position.theta=-1;
    */
}
