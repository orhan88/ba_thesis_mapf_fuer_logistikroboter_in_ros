...
Instance::Instance(const mapf_interfaces::msg::MAPFInstance::SharedPtr instance_msg)
{
	printf("INSTANCE HERE: STEP = '%d'", instance_msg->step);
	loadMapFromMsg(instance_msg);
	loadAgentsFromMsg(instance_msg);
...
void Instance::loadMapFromMsg(const mapf_interfaces::msg::MAPFInstance::SharedPtr instance_msg)
{
	//save the map size (x,y) from the MAPFInstance.msg
	num_of_rows = instance_msg->map.size_y;
	num_of_cols = instance_msg->map.size_x;
	
	//create a map where all positions are free
	//note: map is 1D (1dimensional), size of the map = number of rows * number of columns
	map_size = num_of_cols * num_of_rows;
	my_map.resize(map_size, false);
	
	//get the blocked positions from the MAPFInstance.msg and save it in the blocked list
	int blocked_size = instance_msg->map.blocked.size();
	vector <mapf_interfaces::msg::Position> blocked_list = instance_msg->map.blocked; 

	//iterate over each element in the vector "blocked" and save the blocked position in the map
	for (auto &position : blocked_list)
	{
		my_map[linearizeCoordinate(position.pos_y-1, position.pos_x-1)] = true;
	}
}

void Instance::loadAgentsFromMsg(const mapf_interfaces::msg::MAPFInstance::SharedPtr instance_msg)
{
	num_of_agents = instance_msg->map.number_robots;
	start_locations.resize(num_of_agents);
	goal_locations.resize(num_of_agents);

	vector <mapf_interfaces::msg::RobotInfo> robots = instance_msg->robots; 
	int row, col, count = 0;
	for (auto &robot : robots)
	{
		//in simulation grid starts with 1, in scene file as here in calculation with 0	
		row = robot.position.pos_y-1; 
		col = robot.position.pos_x-1;
		start_locations[count] = linearizeCoordinate(row, col);
		
		//get only the first checkpoint and take it as the only goal
		row = robot.checkpoints.at(0).position.pos_y-1;
		col = robot.checkpoints.at(0).position.pos_x-1;
		goal_locations[count] = linearizeCoordinate(row, col);
		count++;
	}
}
